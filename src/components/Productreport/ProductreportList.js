...
const ProductList = ({ productTransaction }) => {
  ...
  const handleDownloadCSV = () => {
    const headers = ['User Name', 'Product Name', 'Price', 'Quantity', 'Total', 'Location', 'Date/Time'];
    const csvRows = [
  ...
  const handleDownloadPDF = () => {
    ...
    filteredTransaction.forEach((transaction) => {
      ...
      doc.text(`${transaction.transaction.quantity * transaction.product.price}`, columns.total, row);
        ...
        {filteredTransaction.length > 0 ? (
          filteredTransaction.map((transaction) => (
            <div key={transaction.transaction.id} className="productreportlist-table-row">
              <span>{transaction.user_details?.firstName} {transaction.user_details?.lastName}</span>
              <span>{transaction.product.name}</span>
              <span>{transaction.product.price}</span>
              <span>{transaction.transaction.quantity}</span>
              <span>{transaction.transaction.quantity * transaction.product.price}</span>
