...
const ServiceStep = ({ stats, selectedLocation }) => {
  ...
  const refreshTransactionOfCustomer = async () => {
    try {
      ...
      const formattedProductTransactions = productData.map((transaction) => ({
        ...
        type: transaction?.product?.type ? transaction?.product?.type : 'product',
  ...
  const handleSort = (column) => {
    const newDirection = sortColumn === column && sortDirection === 'asc' ? 'dsc' : 'asc';
        ...
        <div className='service-info'>
          {customer ? (
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                <p>
                  <span>Minutes Balance:</span> {customer.profile?.available_balance}
                </p>
                <p>
                  <span>Total Spent:</span> £{customer?.total_service_purchased_price?.toFixed(2)}
                </p>
              </div>
            </div>
          ) : (
          ...
          {customer && (
            <div className='use-btn'>
              <button className='confirm-button' onClick={() => setBuyProductModal(true)}>
                Buy Product
              </button>
              <button className='confirm-button' onClick={() => setBuyServiceModal(true)}>
                Buy Service
              </button>
                ...
                combinedTransactions.map((transaction) => (
                  <div key={transaction.id} className='transaction-table-row'>
                    <span data-label='Date/Time'>
                      {transaction?.createdAt ? `${formatDate(transaction?.createdAt)} ${transaction?.createdAt.split(' ')[1]}` : '-'}
                    </span>
                    <span data-label='Type'>
                      <span className={`transaction-type ${transaction?.type}`}>
                        {transaction?.type ? (transaction?.type === 'used' ? 'Used Min.' : 'Service') : '-'}
                      </span>
                    </span>
                    <span data-label='Product / Service'>
                      {transaction.productName ? transaction?.productName : '-'}
                    </span>
                    <span data-label='Price' className='tbprice'>
                      {typeof transaction?.price === 'number' ? `£${transaction.price.toFixed(2)}` : '-'}
                    </span>
                    <span data-label='Quantity' className='tbprice'>
